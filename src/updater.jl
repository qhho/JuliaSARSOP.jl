function update(tree::SARSOPTree, b::Vector{Float64}, a::Int, o::Int)
    bp = zero(b)
...
function _update!(bp, tree::SARSOPTree, b::Vector{Float64}, a::Int, o::Int)
    bp_sum = 0.0
    pomdp = tree.pomdp

    for s in states(tree)
        if b[s] > 0.0
            td = transition(pomdp, s, a)
            for (sp, tp) in weighted_iterator(td)
                sp = stateindex(pomdp, sp)
